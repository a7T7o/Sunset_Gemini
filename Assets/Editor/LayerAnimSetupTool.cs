...
{
    [MenuItem("Tools/手持三向生成流程/图层动画设置")]
    static void ShowWindow()
    {
        GetWindow<LayerAnimSetupTool>("图层动画设置");
    }
    ...
    void CreateAnimationClipFromTexture(Texture2D texture, string outputPath, string animName)
    {
        ...
        {
            // 最后一个sprite出现在 lastFrame 帧位置
            // 动画总时长 = totalFrames / 60fps
            float time = (i * (float)lastFrame / (sprites.Count - 1)) / 60f;
